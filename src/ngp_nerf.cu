...
// NerfShotApp::NetworkDims NerfShotApp::network_dims_nerf() const {
// 	NetworkDims dims;
...
// void NerfShotApp::NerfTracer::init_rays_from_camera(
// 	uint32_t sample_index,
// 	uint32_t padded_output_width,
// 	uint32_t n_extra_dims,
...
// uint32_t NerfShotApp::NerfTracer::trace(
...
// void NerfShotApp::NerfTracer::enlarge(size_t n_elements, uint32_t padded_output_width, uint32_t n_extra_dims, cudaStream_t stream) {
...
// void NerfShotApp::Nerf::Training::reset_extra_dims(default_rng_t& rng) {
...
// void NerfShotApp::Nerf::Training::update_extra_dims() {
// 	uint32_t n_extra_dims = dataset.n_extra_dims();
...
// const float* NerfShotApp::get_inference_extra_dims(cudaStream_t stream) const {
...
// void NerfShotApp::Nerf::Training::set_camera_intrinsics(int frame_idx, float fx, float fy, float cx, float cy, float k1, float k2, float p1, float p2, float k3, float k4, bool is_fisheye) {
...
// void NerfShotApp::Nerf::Training::set_camera_extrinsics_rolling_shutter(int frame_idx, mat4x3 camera_to_world_start, mat4x3 camera_to_world_end, const vec4& rolling_shutter, bool convert_to_ngp) {
// 	if (frame_idx < 0 || frame_idx >= dataset.n_images) {
// 		return;
// 	}
...
// void NerfShotApp::Nerf::Training::set_camera_extrinsics(int frame_idx, mat4x3 camera_to_world, bool convert_to_ngp) {
// 	set_camera_extrinsics_rolling_shutter(frame_idx, camera_to_world, camera_to_world, vec4(0.0f), convert_to_ngp);
// }

// void NerfShotApp::Nerf::Training::reset_camera_extrinsics() {
...
// void NerfShotApp::Nerf::Training::export_camera_extrinsics(const fs::path& path, bool export_extrinsics_in_quat_format) {
...
// mat4x3 NerfShotApp::Nerf::Training::get_camera_extrinsics(int frame_idx) {
// 	if (frame_idx < 0 || frame_idx >= dataset.n_images) {
// 		return mat4x3(1.0f);
// 	}
...
// void NerfShotApp::Nerf::Training::update_transforms(int first, int last) {
// 	if (last < 0) {
...
// void NerfShotApp::create_empty_nerf_dataset(size_t n_images, int aabb_scale, bool is_hdr) {
// 	m_data_path = {};
...
// void NerfShotApp::load_nerf_post() { // moved the second half of load_nerf here
...
// void NerfShotApp::load_nerf(const fs::path& data_path) {
// 	if (!data_path.empty()) {
...
// void NerfShotApp::update_density_grid_nerf(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
...
// void NerfShotApp::update_density_grid_mean_and_bitfield(cudaStream_t stream) {
...
// void NerfShotApp::mark_density_grid_in_sphere_empty(const vec3& pos, float radius, cudaStream_t stream) {
// 	const uint32_t n_elements = NERF_GRID_N_CELLS() * (m_nerf.max_cascade + 1);
...
// void NerfShotApp::NerfCounters::prepare_for_training_steps(cudaStream_t stream) {
...
// float NerfShotApp::NerfCounters::update_after_training(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
...
// void NerfShotApp::train_nerf(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
...
// void NerfShotApp::train_nerf_step(uint32_t target_batch_size, NerfShotApp::NerfCounters& counters, cudaStream_t stream) {
...
// void NerfShotApp::training_prep_nerf(uint32_t batch_size, cudaStream_t stream) {
// 	if (m_nerf.training.n_images_for_training == 0) {
// 		return;
// 	}
...
// void NerfShotApp::optimise_mesh_step(uint32_t n_steps) {
...
// void NerfShotApp::compute_mesh_vertex_colors() {
// 	uint32_t n_verts = (uint32_t)m_mesh.verts.size();
// 	if (!n_verts) {
// 		return;
// 	}
...
// GPUMemory<float> NerfShotApp::get_density_on_grid(ivec3 res3d, const BoundingBox& aabb, const mat3& render_aabb_to_local) {
...
// GPUMemory<vec4> NerfShotApp::get_rgba_on_grid(ivec3 res3d, vec3 ray_dir, bool voxel_centers, float depth, bool density_as_alpha) {
// 	const uint32_t n_elements = (res3d.x*res3d.y*res3d.z);
...
// int NerfShotApp::marching_cubes(ivec3 res3d, const BoundingBox& aabb, const mat3& render_aabb_to_local, float thresh) {
...
// uint8_t* NerfShotApp::Nerf::get_density_grid_bitfield_mip(uint32_t mip) {
// 	return density_grid_bitfield.data() + grid_mip_offset(mip)/8;
// }

// int NerfShotApp::find_best_training_view(int default_view) {
