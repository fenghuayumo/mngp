...
// #include <neural-graphics-primitives/nerfshot_app.h>
...
// void NerfShotApp::update_imgui_paths() {
...
// void NerfShotApp::load_training_data(const fs::path& path) {
// 	if (!path.exists()) {
...
// fs::path NerfShotApp::find_network_config(const fs::path& network_config_path) {
...
// json NerfShotApp::load_network_config(const fs::path& network_config_path) {
...
// void NerfShotApp::reload_network_from_file(const fs::path& path) {
// 	if (!path.empty()) {
...
// void NerfShotApp::reload_network_from_json(const json& json, const std::string& config_base_path) {
...
// void NerfShotApp::load_file(const fs::path& path) {
// 	if (!path.exists()) {
...
// void NerfShotApp::reset_accumulation(bool due_to_camera_movement, bool immediate_redraw) {
// 	if (immediate_redraw) {
// 		redraw_next_frame();
// 	}
...
// void NerfShotApp::set_visualized_dim(int dim) {
// 	m_visualized_dimension = dim;
// 	reset_accumulation();
// }

// void NerfShotApp::translate_camera(const vec3& rel, const mat3& rot, bool allow_up_down) {
...
// void NerfShotApp::set_nerf_camera_matrix(const mat4x3& cam) {
// 	m_camera = m_nerf.training.dataset.nerf_matrix_to_ngp(cam);
// }

// vec3 NerfShotApp::look_at() const {
// 	return view_pos() + view_dir() * m_scale;
// }

// void NerfShotApp::set_look_at(const vec3& pos) {
// 	m_camera[3] += pos - look_at();
// }

// void NerfShotApp::set_scale(float scale) {
// 	auto prev_look_at = look_at();
// 	m_camera[3] = (view_pos() - prev_look_at) * (scale / m_scale) + prev_look_at;
// 	m_scale = scale;
// }

// void NerfShotApp::set_view_dir(const vec3& dir) {
...
// void NerfShotApp::next_training_view() {
...
// void NerfShotApp::add_training_views_to_camera_path() {
...
// void NerfShotApp::set_camera_to_training_view(int trainview) {
// 	auto old_look_at = look_at();
...
// void NerfShotApp::set_train(bool mtrain) {
...
// void NerfShotApp::compute_and_save_marching_cubes_mesh(const fs::path& filename, ivec3 res3d , BoundingBox aabb, float thresh, bool unwrap_it) {
...
// ivec3 NerfShotApp::compute_and_save_png_slices(const fs::path& filename, int res, BoundingBox aabb, float thresh, float density_range, bool flip_y_and_z_axes) {
// 	mat3 render_aabb_to_local = mat3(1.0f);
// 	if (aabb.is_empty()) {
// 		aabb =  m_render_aabb ;
// 		render_aabb_to_local = m_render_aabb_to_local;
...
// template <typename T>
// void NerfShotApp::dump_parameters_as_images(const T* params, const std::string& filename_base) {
// 	if (!m_network) {
// 		return;
// 	}
...
// template void NerfShotApp::dump_parameters_as_images<__half>(const __half*, const std::string&);
// template void NerfShotApp::dump_parameters_as_images<float>(const float*, const std::string&);

// mat4x3 NerfShotApp::crop_box(bool nerf_space) const {
...
// void NerfShotApp::set_crop_box(mat4x3 m, bool nerf_space) {
// 	if (nerf_space) {
...
// std::vector<vec3> NerfShotApp::crop_box_corners(bool nerf_space) const {
...
// 				"NerfShotApp.background_color = [%0.3f, %0.3f, %0.3f, %0.3f]\n"
// 				"NerfShotApp.exposure = %0.3f\n"
// 				"NerfShotApp.sun_dir = [%0.3f,%0.3f,%0.3f]\n"
// 				"NerfShotApp.up_dir = [%0.3f,%0.3f,%0.3f]\n"
// 				"NerfShotApp.view_dir = [%0.3f,%0.3f,%0.3f]\n"
// 				"NerfShotApp.look_at = [%0.3f,%0.3f,%0.3f]\n"
// 				"NerfShotApp.scale = %0.3f\n"
// 				"NerfShotApp.fov,NerfShotApp.aperture_size,NerfShotApp.slice_plane_z = %0.3f,%0.3f,%0.3f\n"
// 				"NerfShotApp.autofocus_target = [%0.3f,%0.3f,%0.3f]\n"
// 				"NerfShotApp.autofocus = %s\n\n"
// 				, b.r, b.g, b.b, b.a
// 				, m_exposure
// 				, s.x, s.y, s.z
// 				, u.x, u.y, u.z
...
// 			bool disable_marching_cubes = false;
...
// void NerfShotApp::visualize_nerf_cameras(ImDrawList* list, const mat4& world2proj) {
...
// void NerfShotApp::draw_visualizations(ImDrawList* list, const mat4x3& camera_matrix) {
...
// bool NerfShotApp::keyboard_event() {
...
// 	bool shift = ImGui::GetIO().KeyMods & ImGuiMod_Shift;
...
// 	if (ImGui::IsKeyPressed(ImGuiKey_Minus) /** || ImGui::IsKeyPressed('_')*/) {
// 		if (m_fps_camera) {
// 			m_camera_velocity /= 1.5f;
// 		} else {
// 			set_scale(m_scale / 1.1f);
...
// mat3 NerfShotApp::rotation_from_angles(const vec2& angles) const {
// 	vec3 up = m_up_dir;
// 	vec3 side = m_camera[0];
// 	return rotmat(angles.x, up) * rotmat(angles.y, side);
// }

// void NerfShotApp::mouse_drag() {
...
// bool NerfShotApp::begin_frame() {
// 	if (glfwWindowShouldClose(m_glfw_window) || ImGui::IsKeyPressed(ImGuiKey_Escape) || ImGui::IsKeyPressed(ImGuiKey_Q)) {
// 		destroy_window();
// 		return false;
// 	}
...
// void NerfShotApp::handle_user_input() {
// 	if (ImGui::IsKeyPressed(ImGuiKey_Tab) || ImGui::IsKeyPressed(ImGuiKey_GraveAccent)) {
...
// vec3 NerfShotApp::vr_to_world(const vec3& pos) const {
// 	return mat3(m_camera) * pos * m_scale + m_camera[3];
// }

// void NerfShotApp::begin_vr_frame_and_handle_vr_input() {
// 	if (!m_hmd) {
...
// void NerfShotApp::SecondWindow::draw(GLuint texture) {
// 	if (!window)
// 		return;
...
// void NerfShotApp::init_opengl_shaders() {
...
// void NerfShotApp::blit_texture(const Foveation& foveation, GLint rgba_texture, GLint rgba_filter_mode, GLint depth_texture, GLint framebuffer, const ivec2& offset, const ivec2& resolution) {
// 	if (m_blit_program == 0) {
// 		return;
// 	}
...
// void NerfShotApp::prepare_next_camera_path_frame() {
// 	if (!m_camera_path.rendering) {
// 		return;
// 	}
...
// void NerfShotApp::train_and_render(bool skip_rendering) {
// 	if (m_train) {
...
// #ifdef NGP_GUI
// void NerfShotApp::create_second_window() {
...
// 	}
// 	m_second_window.window = glfwCreateWindow(win_w, win_h, "Fullscreen Output", NULL, m_glfw_window, nullptr);
...
// void NerfShotApp::set_n_views(size_t n_views) {
...
// void NerfShotApp::init_window(int resw, int resh, bool hidden, bool second_window) {
// #ifndef NGP_GUI
...
// 	m_glfw_window = glfwCreateWindow(m_window_res.x, m_window_res.y, title.c_str(), NULL, NULL, nullptr);
...
// 		NerfShotApp* nerfShot_app = (NerfShotApp*)glfwGetWindowUserPointer(window);
// 		if (!nerfShot_app) {
// 			return;
// 		}

// 		nerfShot_app->redraw_gui_next_frame();
// 		for (int i = 0; i < count; i++) {
// 			nerfShot_app->load_file(paths[i]);
// 		}
// 	});
...
// 		NerfShotApp* nerfShot_app = (NerfShotApp*)glfwGetWindowUserPointer(window);
// 		if (nerfShot_app) {
// 			nerfShot_app->redraw_next_frame();
// 		}
// 	});
...
// void NerfShotApp::destroy_window() {
// #ifndef NGP_GUI
...
// void NerfShotApp::update_vr_performance_settings() {
// #ifdef NGP_GUI
// 	if (m_hmd) {
...
// fs::path NerfShotApp::training_data_path() const {
// 	return m_data_path.with_extension("training");
// }

// bool NerfShotApp::want_repl() {
// 	bool b = m_want_repl;
// 	m_want_repl = false;
// 	return b;
// }

// void NerfShotApp::apply_camera_smoothing(float elapsed_ms) {
...
// CameraKeyframe NerfShotApp::copy_camera_to_keyframe() const {
// 	return CameraKeyframe(m_camera, m_slice_plane_z, m_scale, fov(), m_aperture_size, m_nerf.glow_mode, m_nerf.glow_y_cutoff);
// }

// void NerfShotApp::set_camera_from_keyframe(const CameraKeyframe& k) {
// 	m_camera = k.m();
...
// void NerfShotApp::set_camera_from_time(float t) {
// 	if (m_camera_path.keyframes.empty()) {
// 		return;
// 	}
...
// uint32_t NerfShotApp::network_width(uint32_t layer) const {
// 	return m_network ? m_network->width(layer) : 0;
// }

// uint32_t NerfShotApp::network_num_forward_activations() const {
// 	return m_network ? m_network->num_forward_activations() : 0;
// }

// void NerfShotApp::set_max_level(float maxlevel) {
// 	if (!m_network) return;
...
// NerfShotApp::NetworkDims NerfShotApp::network_dims() const {
// 	return network_dims_nerf();
// }

// void NerfShotApp::reset_network(bool clear_density_grid) {
...
// NerfShotApp::NerfShotApp() {
// 	if (!(__CUDACC_VER_MAJOR__ > 10 || (__CUDACC_VER_MAJOR__ == 10 && __CUDACC_VER_MINOR__ >= 2))) {
// 		throw std::runtime_error{"NerfShotApp requires CUDA 10.2 or later."};
// 	}
...
// 		GLFWwindow* offscreen_context = glfwCreateWindow(640, 480, "", NULL, NULL, NULL);
...
// bool NerfShotApp::clear_tmp_dir() {
// 	wait_all(m_render_futures);
// 	m_render_futures.clear();
...
// void NerfShotApp::train(uint32_t batch_size) {
...
// 	uint32_t n_prep_to_skip = tcnn::clamp(m_training_step / 16u, 1u, 16u) ;
...
// vec2 NerfShotApp::calc_focal_length(const ivec2& resolution, const vec2& relative_focal_length, int fov_axis, float zoom) const {
// 	return relative_focal_length * (float)resolution[fov_axis] * zoom;
// }

// vec2 NerfShotApp::render_screen_center(const vec2& screen_center) const {
...
// void NerfShotApp::render_frame_main(
// 	CudaDevice& device,
// 	const mat4x3& camera_matrix0,
// 	const mat4x3& camera_matrix1,
// 	const vec2& orig_screen_center,
...
// void NerfShotApp::render_frame_epilogue(
// 	cudaStream_t stream,
// 	const mat4x3& camera_matrix0,
// 	const mat4x3& prev_camera_matrix,
// 	const vec2& orig_screen_center,
...
// float NerfShotApp::get_depth_from_renderbuffer(const CudaRenderBuffer& render_buffer, const vec2& uv) {
...
// vec3 NerfShotApp::get_3d_pos_from_pixel(const CudaRenderBuffer& render_buffer, const ivec2& pixel) {
...
// NerfShotApp::LevelStats compute_level_stats(const float* params, size_t n_params) {
// 	NerfShotApp::LevelStats s = {};
// 	for (size_t i = 0; i < n_params; ++i) {
// 		float v = params[i];
// 		float av = fabsf(v);
// 		if (av < 0.00001f) {
...
// void NerfShotApp::gather_histograms() {
// 	if (!m_network) {
// 		return;
// 	}
...
// void NerfShotApp::save_snapshot(const fs::path& path, bool include_optimizer_state, bool compress) {
...
// void NerfShotApp::load_snapshot(const fs::path& path) {
...
// void NerfShotApp::CudaDevice::set_nerf_network(const std::shared_ptr<NerfNetwork<precision_t>>& nerf_network) {
// 	m_network = m_nerf_network = nerf_network;
// }

// void NerfShotApp::sync_device(CudaRenderBuffer& render_buffer, NerfShotApp::CudaDevice& device) {
// 	if (!device.dirty()) {
// 		return;
// 	}
...
// ScopeGuard NerfShotApp::use_device(cudaStream_t stream, CudaRenderBuffer& render_buffer, NerfShotApp::CudaDevice& device) {
...
// void NerfShotApp::set_all_devices_dirty() {
// 	for (auto& device : m_devices) {
// 		device.set_dirty(true);
// 	}
// }

// void NerfShotApp::load_camera_path(const fs::path& path) {
// 	m_camera_path.load(path, mat4x3(1.0f));
// }

// bool NerfShotApp::loop_animation() {
// 	return m_camera_path.loop;
// }

// void NerfShotApp::set_loop_animation(bool value) {
