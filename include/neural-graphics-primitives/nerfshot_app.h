...
public:
	...
	NerfShotApp(const fs::path& data_path) : NerfShotApp() { load_training_data(data_path); }
	NerfShotApp(const fs::path& data_path, const fs::path& network_config_path) : NerfShotApp(data_path) { reload_network_from_file(network_config_path); }
	NerfShotApp(const fs::path& data_path, const nlohmann::json& network_config) : NerfShotApp(data_path) { reload_network_from_json(network_config); }
